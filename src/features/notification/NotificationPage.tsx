...
import { Bot, Shield, Bell, Activity, RefreshCcw } from "lucide-react";
...
export default function NotificationsPage({ lang = "en" }: { lang?: "en" | "id" }) {
  ...
  const telegramState = !botConfigured
    ? t("Awaiting Setup", "Menunggu Setup")
    : !webhookEnabled
      ? t("Webhook Disabled", "Webhook Nonaktif")
      : webhookStatus === "missing"
        ? t("Webhook Missing", "Webhook Hilang")
        : !webhookUrlMatch
          ? t("Webhook Mismatch", "Webhook Tidak Cocok")
          : t("Connected", "Terhubung");

  const stateClass =
    telegramState === t("Connected", "Terhubung")
  ...
  const getResultLabel = (res: string) => {
    ...
    if (r === "success") return t("SUCCESS", "BERHASIL");
    if (r === "acknowledged") return t("ACKNOWLEDGED", "DITERIMA");
    if (r === "failed") return t("FAILED", "GAGAL");
    return res.toUpperCase();
  };

  const getCommandLabel = (cmd: string) => {
    const c = cmd.toUpperCase();
    if (c === "AUTO_REPLY") return t("Inbound Tracking", "Pelacakan Masuk");
    if (c === "OPEN") return t("OPEN", "BUKA");
    if (c === "CLOSE") return t("CLOSE", "TUTUP");
    if (c === "AUTO") return t("AUTO", "OTOMATIS");
    return cmd;
  };
        ...
        <header className="relative overflow-hidden rounded-[2.5rem] bg-white dark:bg-slate-900/50 p-8 md:p-10 shadow-2xl shadow-teal-500/5 border border-slate-200/60 dark:border-white/5 backdrop-blur-sm">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-2">
              ...
              <h1 className="text-5xl md:text-6xl font-black text-slate-800 dark:text-white tracking-tighter">{t("System Alerts", "Pemberitahuan Sistem")}</h1>
            </div>
          ...
          <div className="flex flex-col lg:col-span-12">
            <section className="flex-1 flex flex-col rounded-[2.5rem] bg-white dark:bg-slate-900/40 p-10 shadow-xl border border-slate-200/60 dark:border-white/5 backdrop-blur-sm">
              <div className="flex items-center gap-3 mb-10">
                ...
                <h2 className="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">{t("Activity Log", "Log Aktivitas")}</h2>
              </div>
              ...
              <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                {auditLogs.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-20 opacity-30">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest text-center">{t("No Delivery Records Found", "Tidak Ada Catatan Pengiriman Ditemukan")}</p>
                  </div>
                ) : (
                  auditLogs.map((log) => (
                    <div key={log.id} className="p-6 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-200/50 dark:border-white/5 group hover:border-teal-500/30 transition-all">
                      <div className="flex items-center justify-between mb-3">
                        <p className="text-base font-black text-slate-800 dark:text-white tracking-tight">{getCommandLabel(log.command)}</p>
            ...
            <section className="flex-1 flex flex-col rounded-[2.5rem] bg-white dark:bg-slate-900/40 p-10 shadow-xl border border-slate-200/60 dark:border-white/5 backdrop-blur-sm">
              <div className="flex items-center justify-between mb-10">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-xl font-bold text-slate-800 dark:text-white tracking-tight">{t("Diagnostics", "Diagnostik")}</h2>
                </div>
                <button onClick={() => handleRepair()} className="text-[10px] font-black text-teal-600 hover:text-teal-700 uppercase tracking-widest transition-colors">{t("Sync", "Sinkronisasi")}</button>
              </div>

              <div className="grid grid-cols-1 gap-4">
                <DiagRow label={t("Integration", "Integrasi")} value={telegramMode.toUpperCase()} active={telegramMode === 'notification-only'} />
                <DiagRow label={t("Bot Config", "Konfigurasi Bot")} value={botConfigured ? t("VALID", "VALID") : t("MISSING", "TIDAK ADA")} active={botConfigured} />
                <DiagRow label={t("Default Chat", "Obrolan Utama")} value={defaultChatConfigured ? t("ACTIVE", "AKTIF") : t("MISSING", "TIDAK ADA")} active={defaultChatConfigured} />
                <DiagRow label={t("Webhook", "Webhook")} value={webhookUrlMatch ? t("MATCH", "COCOK") : t("MISMATCH", "TIDAK COCOK")} active={webhookUrlMatch} />
                <DiagRow label={t("Groups", "Grup")} value={`${allowedGroupsCount} ${t("AUTHORIZED", "DIIZINKAN")}`} active={allowedGroupsCount > 0} />
              </div>
