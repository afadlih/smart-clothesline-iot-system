  ...
  return (
    <div className="w-full max-w-md rounded-3xl border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 p-6 shadow-xl backdrop-blur transition-all duration-300 relative">
      <div className="absolute top-4 right-4 flex items-center gap-1.5">
        ...
        <span className="text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest">{t("Interactive Preview", "Pratinjau Interaktif")}</span>
      </div>

      <div className="mb-6">
        <div className="font-extrabold text-slate-800 dark:text-white text-base">{t("Your Smart Clothesline", "Jemuran Pintar Anda")}</div>
      ...
      <div className="mb-6 p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200/50 dark:border-white/5 flex items-center justify-between">
        <span className="text-xs font-bold text-slate-600 dark:text-slate-450">{t("Clothesline Cover:", "Penutup Jemuran:")}</span>
        <span className={`px-4 py-1.5 rounded-full text-xs font-black transition-all ${
          ...
          {clotheslineState === "OPEN" ? t("☀️ OPEN (Drying)", "☀️ TERBUKA (Menjemur)") : t("🌧️ CLOSED (Protected)", "🌧️ TERTUTUP (Terlindungi)")}
        </span>
      </div>
        ...
        <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200/40 dark:border-white/5 flex items-center gap-3">
          ...
          <div>
            ...
            <p className={`text-xs font-black ${isRainy ? 'text-rose-500' : 'text-emerald-500'}`}>
              {isRainy ? t("Raining", "Hujan") : t("Clear & Sunny", "Cerah")}
            </p>
          </div>
        </div>
      ...
      >
        {isRainy ? t("☀️ Return to clear weather", "☀️ Kembali ke cuaca cerah") : t("🌧️ Simulate sudden rain", "🌧️ Simulasikan hujan mendadak")}
      </button>

      {/* Small Note */}
      <div className="mt-3 text-[10px] text-center text-slate-500 dark:text-slate-450 leading-normal">
        {t(
          "This is a visual demo. Real device behavior depends on your connected hardware and settings.",
          "Ini adalah demo visual. Perilaku perangkat asli bergantung pada perangkat keras yang terhubung dan pengaturan Anda."
        )}
      </div>
    </div>
  );
